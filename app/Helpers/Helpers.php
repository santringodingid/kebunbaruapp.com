...
if (!function_exists('masehi')) {
    function masehi(): string
    {
...
if (!function_exists('hijri')) {
    ...
    {
        $now = Carbon::now();
        if ($now->toTimeString() >= '17:30:00') {
            $now = $now->addDays(1)->toDateString();
        }else{
            $now = $now->toDateString();
        }

        $hijri = \App\Models\SettingManagement\Hijri::query()->where('masehi', $now)->first();
        if ($hijri) {
            return $hijri->hijri;
        }
...
if (!function_exists('checkRegion')) {
    function checkRegion($id, $v, $d, $c, $p): array
    {
        $checkRegion = Region::query()->find($id);
        if (!$checkRegion) {
            $village = Village::query()->find($v);
            $district = District::query()->find($d);
            $city = City::query()->find($c);
            $province = Province::query()->find($p);
            ...
            Region::query()->create([
                ...
                'portal_code' => $village->portal_code
            ]);
